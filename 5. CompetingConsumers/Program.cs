...
namespace CompetingConsumers
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Utils.ConsoleWriteHeader("Testing Competing Consumers Pattern", ConsoleColor.Red);
            ...
            //in this part will simulate multiple consumers that will consume messages from the service bus in an asynchrounous manner
            //note 1: the code sample below uses a series of Tasks to sumlate the processing in the multithreaded way. this is for illustration purposes only.
            //in real life scenarios, you should use different consumers with intependend processing powers, such as VMs, Worker roles, or better, Azure Functions. 
            //there is no real gains or benefits in processing the queue in multiple threads from a single machine/service
            ...
            //building the consumers. will create 10 separate consumers.
            for (var i = 0; i < 10; i++)
            {
                        ...
                        try
                        {
                            ...
                            if (message != null)
                            {
                                ...
                                try
                                {
                                    ...
                                    //each color belongs to a different thread/consumer. since there are only a limited number of availabel colors, a color may be repeated in other threads 

                                    //Console.WriteLine(s, c);
                                    Console.WriteLine($"Message Seq: {message.SystemProperties.SequenceNumber}", c);
                                }
                                finally
                                {
                                    _semaphore.Release();
            ...
            //process messages by all consumers
